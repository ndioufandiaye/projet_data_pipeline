...
def _():
    ...
    return Producer, Consumer, KafkaError, SparkSession, col, current_timestamp, lit, mo, pd, json
...
def _():
    ...
    return KAFKA_CONF, TOPIC_MAIN, TOPIC_ALERTS, S3_ENDPOINT, S3_ACCESS_KEY, S3_SECRET_KEY

@app.cell(hide_code=True)
def _(mo):
    mo.md("## 1️⃣ Ingestion Temps Réel (Producer)")
    return

@app.cell
def _(KAFKA_CONF, json, Producer):
    ...
    return stream_data_to_kafka,

@app.cell(hide_code=True)
def _(mo):
    mo.md("## 2️⃣ Traitement Medallion avec Spark (Batch/Micro-batch)")
    return
...
@app.cell
def _(KAFKA_CONF, json, Producer, time):
